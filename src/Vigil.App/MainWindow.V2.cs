...
public partial class MainWindow
{
    ...
    private sealed class GoalRow { public GoalRow(GoalRecord value) => Value = value; public GoalRecord Value { get; } public override string ToString() { var value = Value; var prefix = value.Status switch { GoalStatus.Completed => "✓ ", GoalStatus.Incomplete => "○ ", GoalStatus.Paused => "⏸ ", GoalStatus.Abandoned => "× ", _ => "" }; return $"{prefix}[{HorizonName(value.Horizon)}] {value.Title}\n{StatusName(value.Status)}{(value.DueAt is null ? "" : $" · 截止 {value.DueAt:yyyy-MM-dd}")}"; } }
    ...
    private static string StatusName(GoalStatus s) => s switch { GoalStatus.NotStarted => "未开始", GoalStatus.InProgress => "进行中", GoalStatus.Incomplete => "未完成", GoalStatus.Completed => "已完成", GoalStatus.Paused => "暂停", GoalStatus.Abandoned => "放弃", _ => "归档" };
